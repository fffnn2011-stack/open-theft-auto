# PROMPT: Build "Open Theft Auto" — a complete GTA-style open-world sandbox — end to end

You are a senior game engineer. Build a complete, playable, GTA-style 3D open-world
sandbox game called **Open Theft Auto** ("Free Harbor") from scratch, end to end, to the
specification at the bottom of this document. The spec is exhaustive and was reverse-
engineered from a finished, working game — treat every number in it (speeds, prices,
damages, probabilities, dimensions, timers) as the required tuning, not a suggestion.
Where the spec is silent, make the choice that best matches GTA-V-style "muted realism"
and note it in a DEVIATIONS.md.

## 1. Hard constraints (non-negotiable)

1. **Engine:** Godot 4.7+ (standard build, NOT .NET). Everything in **pure GDScript**.
   No C#, no GDExtension, no third-party plugins, no build step. The project must run by
   opening the folder in Godot and pressing F5.
2. **No physics bodies.** All movement (player, cars, aircraft, NPCs, bullets,
   particles) is pure kinematic: positions integrated by hand each frame, collision via
   a custom AABB query `collides_at(x, z, radius, altitude) -> bool` backed by a spatial
   hash (24 m cells) over `{x, z, w, d, h}` boxes, with altitude gating so fliers pass
   over rooftops, plus walkable-zone whitelists and water/coastline blocks. The single
   exception: the Moon's terrain uses one StaticBody3D trimesh collider (for the buggy
   and spacecraft); the on-foot player samples an analytic `moon_height(x, z)` function.
3. **Everything procedural.** The entire city, airport, islands, Moon, F1 circuit,
   buildings, props, and ALL UI are generated in code. No .tscn files for UI (build
   Control trees in `_build()` methods), no textures except a tiling road/footpath
   texture you may generate, no Theme resource. Building windows must be a single
   emissive MultiMesh (one draw call). Materials are cached/shared (`cmat`) and treated
   as immutable.
4. **Asset substitution.** The original game used ~10 licensed glTF models (McLaren F1,
   Boeing 787, Black Hawk, Space Shuttle 3-piece stack, fighter spacecraft, 5 rigged
   human characters). You do NOT have these files. Build procedural stand-ins with the
   same gameplay envelope: box-grammar cars per style (the spec describes the profiles),
   a procedural jet/helicopter/shuttle (the shuttle MUST be three separately detachable
   meshes: orbiter, external tank, twin SRBs), and capsule-and-box humanoids with a
   procedural walk cycle (swinging thighs/arms), tintable skin/shirt/pants/hair. Keep
   the code structured so a real glTF can later be dropped in behind the same
   build/fitting function (scale, recentre, drop-to-ground, ride-height, meta for
   steerable front wheels / rotor pivots / hand bone attachment).
5. **Dual input, full parity.** Keyboard+mouse AND game controller must both drive 100%
   of the game, including every menu. All actions are rebindable at runtime (see spec §9
   and the pause-menu controls screen in §11). Gamepad confirm = Cross, back = Circle,
   focus rings on every button.
6. **Determinism guards:** clamp `dt = min(0.05, delta)`; all sim ticks
   (stocks/ventures/forbes/autosave) run on `_process` accumulators with catch-up
   guards.

## 2. Project layout (create exactly this)

```
project.godot            # autoloads (exact order: GameState, Forbes, Ventures,
                         # InputConfig, AudioFX, Gamepad, StockMarket, Garage,
                         # RaceManager, SaveGame), main scene, 1920x1080 fullscreen,
                         # stretch canvas_items/expand
scenes/main.tscn         # Node3D + game.gd — the only scene
scripts/
  game.gd                # orchestrator: player, camera, vehicles, combat, NPCs,
                         # police, VIPs, President, milestones, space flight, HUD feed
  world.gd               # CityWorld: procedural city/airport/islands/Moon/facility,
                         # collision system, surface_height
  track.gd               # F1 circuit (Catmull-Rom, pits, grandstands, paddock)
  car_mesh.gd            # procedural car mesh builder (all styles)
  build.gd               # static mesh/material helpers (box/cyl/sphere/plane,
                         # cached materials, facade/roof/water shaders)
  human.gd               # character builder + procedural walk/sit poses
  weapons.gd             # WeaponDB: the 9-weapon table
  hud.gd                 # boot screen, HUD, minimap, speedometer, toasts, scope,
                         # death/victory screens
  pause_menu.gd          # pause + rebindable controls + gamepad diagram
  ui_nav.gd              # gamepad focus helper (focus rings, focus fencing)
  phone.gd  touch_hud.gd
  stock_terminal.gd  dealership_terminal.gd  realtor_terminal.gd  suit_terminal.gd
  race_terminal.gd  donate_terminal.gd  venture_terminal.gd
  vehicle_catalog.gd  property_catalog.gd  suit_catalog.gd
  autoload/
    game_state.gd  save_game.gd  input_config.gd  gamepad.gd  audio_fx.gd
    stock_market.gd  ventures.gd  forbes.gd  garage.gd  race_manager.gd
tests/
  world_collision_audit.gd/.tscn   # headless regression (see §4)
```

## 3. Build order — phases with verification gates

Build in this order. After EVERY phase the project must boot headless with no script
errors (`godot --headless --quit`) and the collision audit (once it exists, phase 1)
must pass. Do not move to the next phase with a red gate.

- **Phase 0 — Skeleton.** project.godot, main scene, all autoload stubs, boot screen
  with NEW GAME. Gate: boots to title, starts into an empty ground plane.
- **Phase 1 — World + collision.** world.gd districts/roads/river/bridges/parks,
  Build helpers, collision system, surface_height, day/night sky. Write the collision
  audit test now. Gate: audit passes (all road centrelines walkable, river blocked
  below bridge height, coastline blocks, causeway open).
- **Phase 2 — Player + camera + HUD core.** On-foot movement, chase cam, ADS,
  minimap, clock, toasts, pause menu (without rebinding yet), save/load skeleton.
- **Phase 3 — Combat.** WeaponDB, bullets, melee, damage, blood/sparks/pickups,
  death/respawn, sniper scope, aim assist.
- **Phase 4 — NPCs + police.** Pedestrians, traffic, wanted stars, cops/SWAT/vans/
  police helicopter, VIPs + bodyguards.
- **Phase 5 — Cars + racing.** Car handling, dealership, garage/impound, F1 track,
  RaceManager, race terminal, AI racers, race HUD.
- **Phase 6 — Economy.** StockMarket + exchange terminal + trading floor interior +
  bank, Ventures + terminal, Forbes + banners + in-world rivals, hospital donations,
  realtor/safehouses, wealth milestones + celebrations, asset seizure.
- **Phase 7 — Iron Man suit.** Assembly, summon, flight, repulsors/missiles, Stark
  terminal, tiers.
- **Phase 8 — Air & sea.** Boats/jetski/submarine, plane (throttle/stall/gear/
  parachute), helicopter, airport aircraft spawns.
- **Phase 9 — Space.** Hidden facility, Shuttle state machine (booster/tank
  separation, Moon trip, re-entry), fighter spacecraft phase machine, Moon terrain/
  buggy/base, He-3 loop, space vistas, spacecraft nav panel.
- **Phase 10 — Endgame + megaproject.** President motorcade, city takeover, passive
  income, presidential detail, New Harbor Island staged build.
- **Phase 11 — Input & polish.** Full rebinding UI + gamepad diagram, touch HUD,
  audio synthesis (all SFX in spec §10), rumble, phone, final save-field sweep
  (every field in spec §8 persists and restores).

## 4. Verification requirements

- `tests/world_collision_audit.gd`: generates the world headless and asserts —
  every road-grid centreline point walkable; shoreline shallow-wade open; deep water
  blocked; airport causeway open; no collision AABB larger than 220×220; prints
  `WORLD_COLLISION_AUDIT_OK buildings=N` on success. Runnable via
  `godot --headless tests/world_collision_audit.tscn`.
- Headless boot check after every phase: `godot --headless --quit` exits 0 with no
  script errors.
- Manual smoke list per phase (write it in the PR/commit message): what you drove,
  flew, bought, or killed to prove the phase works.
- Autosave round-trip test: start game, earn money, buy a car + stock + venture, quit,
  Continue — all state restored (spec §8 lists every field).

## 5. Acceptance checklist (game is DONE when all are true)

- [ ] Boot → NEW GAME/CONTINUE; mouse captured; controller hot-plugs.
- [ ] Walk, sprint, fight with all 9 weapons; sniper scope zooms over 0.35 s; die →
      WASTED → respawn at safehouse; cash drops to 0 but bank/assets survive.
- [ ] Wanted escalates 1★ cops → 3★ SWAT+vans → 4★ helicopter; ≥3.5 freezes trading;
      ≥4.5 seizure pulses (impound → raid → clawback); bribe via phone for $50k.
- [ ] Steal any street car; buy all 11 catalog vehicles; impound/recover works.
- [ ] Enter Grand Prix from paddock in an F1 car: $100k fee, 3/5/8 laps, bet,
      6-car grid, countdown, rank/lap/drift HUD, ×10/×4/×2 payouts.
- [ ] Boats + submarine (dives with underwater fog); exit only at docks.
- [ ] Plane: spools, needs 14 m/s to lift, stalls, animated gear, belly-landing
      damage, mid-air bail → parachute. Helicopter: vertical takeoff, hover.
- [ ] Iron Man suit assembles plate-by-plate, summons with V, flies, repulsors +
      missiles, bulletproof; 4 purchasable tiers.
- [ ] Stock market ticks live (9 tickers incl. HE3), buy/sell fractional, charts,
      bank deposit/withdraw; ventures: 5-deal flow, negotiate (founders can walk),
      stage raises, exits pay out + respect; Forbes list races you live on downtown
      banners; killing a rival transfers their fortune + 5★.
- [ ] Donations lift Respect/Happiness with diminishing returns; milestones fire once
      each with street celebrations, deferred until on foot on Earth.
- [ ] Hidden facility findable (one cryptic toast at 4 min, no marker): Shuttle does
      ascent → SRB sep 520 m → tank sep → Moon → re-entry splashdown; spacecraft does
      hover → spool → hyper → orbit free-fly → fully manual landing; He-3 mined on
      Moon sells at live HE3 price on Earth; moon buggy drivable on cratered terrain.
- [ ] President motorcade runs; killing him = CITY OWNED, +$5B, $200k/s, police stand
      down, escort detail forms.
- [ ] New Harbor Island purchasable for $500B, builds in 3 stages over ~5 min,
      walkable at stage 3.
- [ ] Every action rebindable (kb + pad) with live DualSense diagram; every screen
      fully navigable by controller alone; touch HUD on mobile export.
- [ ] All spec §10 sounds synthesized (distinct per-gun reports, engine loops, coin,
      sirens, rocket rumble); mute toggle.
- [ ] Autosave every 10 s + on quit; Continue restores everything including live
      market prices and Forbes rival worths.
- [ ] Day/night cycle: sun/fog/sky curves, emissive windows, 72 street lights,
      headlights, beacon pulse; space sky + Earth/Moon vista spheres by altitude.

## 6. Working style

Work phase by phase; commit at each green gate with a message listing the smoke tests
performed. Keep scripts under ~6,000 lines each (split as in §2). Follow the spec's
exact strings for names visible in-game (weapon names, tickers, tycoon names, building
signs, toast copy). Record any intentional deviation in DEVIATIONS.md with a reason.
